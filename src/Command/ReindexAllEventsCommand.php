...
class ReindexAllEventsCommand extends ContainerAwareCommand
{
    ...
    protected function configure()
    {
        $this
            ...
            ->setDescription('Pipes ALL events from the EventStore and reindexes them.')
            ->setHelp(<<<EOF
The <info>%command.name%</info> command will pipe ALL events from the pbjx EventStore
and reindex them into the EventSearch service.

<info>php %command.full_name% --dry-run --tenant-id=client1</info>
...
EOF
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $pbjx->getEventStore()->pipeAllEvents($receiver, $since, $until, $context);
        $this->reindex($queue, $reindexed, $io, $context, $batch, $dryRun, $skipErrors);
        $io->newLine();
